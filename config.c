...
int cpu;
    ...
    {
        /* Adjust zero storage case for a subsystem/device server (MAXCPU 0)
           Allocate minimum storage of one segment */
        storsize = 65536;
    }
    ...
    if (storsize > config_allocmsize)
    {
        ...
        /* Obtain storage */
        /* Hint to page size for cleanest allocation */

        storkeys = calloc((size_t)(storsize >> 12), 4096);
        if (mfree)
            free(mfree);
...
int  cpu;
    ...
    if (xpndsize > config_allocxsize)
    {
        ...
        /* Hint to page boundary */
        xpndstor = calloc((size_t)(xpndsize >> 12), 4096);
